...
import type { RoomEngine } from "./game.js";
import { attachRatings } from "./rating.js";
import {
  parsePeriod,
  leaderboardRows,
  userTournamentMetrics,
  preflopSkillMetrics,
  compareTip
} from "./playerAnalytics.js";
...
function corsOriginHandler(
  ...
  console.warn(
    `[cors] Запрос с Origin "${origin}" отклонён. Добавьте его в FRONTEND_ORIGINS на сервере (точно как в адресной строке, например https://site.ru) или временно FRONTEND_ORIGINS=*`
  );
  callback(null, false);
}
...
function sessionCookieOptions(): { httpOnly: boolean; sameSite: "lax"; secure?: boolean } {
...
/** Данные о докупке для клиента (привязаны к текущему турниру и месту за столом). */
type RebuyClientInfo = {
  enabled: boolean;
  ...
  maxRebuys: number;
  costChips: number;
  stackChipsAwarded: number;
  costMode: "FULL" | "HALF";
  canRebuyNow: boolean;
  blockReason: string | null;
};

function buildRebuyPublicSync(
  tour: Tournament,
  reg: Registration | null,
  seatedAtTable: boolean,
  seatChips: number | null,
  accountChips: number
): RebuyClientInfo {
  const now = Date.now();
...
function mustEliminateZeroStack(tour: Tournament, reg: Registration | undefined): boolean {
  if (!tour.rebuyEnabled) return true;
  const now = Date.now();
  const periodEndMs = tour.startsAt.getTime() + tour.rebuyPeriodMinutes * 60_000;
  if (now > periodEndMs) return true;
...
type Session = { userId: number; username: string; role: Role; activeTournamentId?: number | null };
type ChatMessage = {
  id: string;
  tournamentId: number;
  ts: number;
...
const roomForSession = (me: Session): { tid: number; eng: RoomEngine } => {
  const tid = me.activeTournamentId;
  if (tid == null) throw new Error("Сначала откройте комнату турнира (кнопка «Открыть комнату»).");
  return { tid, eng: getOrCreateRoom(tid) };
};
const sessions = new Map<string, Session>();
const tournamentChat = new Map<number, ChatMessage[]>();
const historyCursorByTournament = new Map<number, number>();
...
const lastTableEmotionAt = new Map<number, number>();
/** Порядок вылета (включая ботов) — для нумерации мест людей. */
const eliminatedOrder = new Map<number, number[]>();
const eliminatedSeen = new Map<number, Set<number>>();

function recordTournamentElimination(tournamentId: number, userId: number): void {
...
function clearTournamentEliminationState(tournamentId: number): void {
  eliminatedOrder.delete(tournamentId);
  eliminatedSeen.delete(tournamentId);
}

/** Данные для экрана победы (клиент); заполняется только при официальном завершении турнира. */
type TournamentVictoryPayload = {
  tournamentTitle: string;
  winnerUserId: number;
  winnerUsername: string;
  winnerChips: number;
  lastHandWinnerLine: string | null;
  lastHandReasonLine: string | null;
  bestHandTitle: string | null;
};

const tournamentVictoryPayloadById = new Map<number, TournamentVictoryPayload>();

function victoryPayloadForFinishedTournament(
  tournamentId: number,
  tour: { finishedAt: Date | null } | null
): TournamentVictoryPayload | null {
  if (!tour?.finishedAt) return null;
  return tournamentVictoryPayloadById.get(tournamentId) ?? null;
}

/** Рейтинговые очки за место (не игровые фишки). */
function placementRatingPoints(place: number): number {
...
async function maybeFinalizeTournament(tournamentId: number): Promise<void> {
  const tour = await prisma.tournament.findUnique({ where: { id: tournamentId } });
  if (!tour || tour.finishedAt) return;
  ...
  if (handsDealt < 1) return;

  const rs = eng.roomState;
  if (rs.handActive || rs.showdownReveal || rs.handResultOverlay) return;
  ...
  if (isBotUserId(winnerId)) {
    ...
    pushChatMessage(
      makeSystemChatMessage(
        tournamentId,
        "Турнир завершён: победитель — бот (демо). Результаты для игроков не зафиксированы."
      )
    );
  ...
  if (humanElim.includes(winnerId)) return;
  ...
  const placements = new Map<number, { place: number; prizeChips: number }>();
  ...
  for (const r of dnsRegs) {
    if (placements.has(r.userId)) continue;
  ...
  pushChatMessage(
    makeSystemChatMessage(
      tournamentId,
      `Турнир завершён! Победитель: @${winnerSeat.username ?? "?"}. Призовые фишки на счёт: ${winnerSeat.chips}.`
    )
  );
  await emitRoomState(tournamentId);
}
...
const makeBotProfile = (chips: number) => {
...
const autoStartAtMs = (startsAt: Date) => startsAt.getTime() + TOURNAMENT_AUTO_START_MS;
const isTournamentActive = (tournamentId: number) => tournamentActiveIds.has(tournamentId);
const isTournamentRoomOpen = (startsAt: Date) => Date.now() >= startsAt.getTime();
const MAX_CHAT_MESSAGES = 300;

const getChatForTournament = (tournamentId: number) => {
...
const pushChatMessage = (msg: ChatMessage) => {
  ...
  rows.push(msg);
  if (rows.length > MAX_CHAT_MESSAGES) rows.splice(0, rows.length - MAX_CHAT_MESSAGES);
  io.to(`tournament:${msg.tournamentId}`).emit("chat_message", msg);
};

const makeSystemChatMessage = (tournamentId: number, text: string): ChatMessage => ({
...
const mapUser = (u: any, rating?: number) => {
  const tid = u.telegramId as string | null | undefined;
  const handle = u.telegramLinkHandle as string | null | undefined;
  const telegramLinked = Boolean(tid);
  const telegramDisplay =
    handle && String(handle).length > 0
      ? `@${handle}`
      : tid
        ? `ID: ${tid}`
        : null;
...
const findLiveApprovedRegistration = async (userId: number, tournamentId?: number) => {
  const now = new Date();
...
const tournamentRoomAccessMessage = async (
  userId: number,
  tournamentId?: number
): Promise<string | null> => {
  ...
  if (!reg) {
    if (tournamentId != null) {
      ...
      if (t.finishedAt) return "Турнир уже завершён.";
...
const collectTournamentBuyIn = async (
  userId: number,
  tournamentId?: number
): Promise<{ ok: true; stackChips: number } | { ok: false; error: string }> => {
  const reg = await findLiveApprovedRegistration(userId, tournamentId);
  if (!reg) return { ok: false, error: "Нет активной регистрации на турнир." };
  if (reg.stackChips != null) return { ok: true, stackChips: reg.stackChips };
...
const persistTournamentStacks = async (rows: { userId: number; chips: number }[]) => {
  const now = new Date();
  const live = await prisma.tournament.findMany({
    where: { startsAt: { lte: now } },
    select: { id: true }
  ...
  for (const { userId, chips } of rows) {
    ...
    if (r) await prisma.registration.update({ where: { id: r.id }, data: { stackChips: chips } });
  }
};
...
const admin = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  if ((req as any).session.role !== "ADMIN") return res.status(403).json({ error: "Forbidden" });
  next();
};

/** Администратор или модератор — просмотр списка игроков и смена статуса аккаунта. */
const staff = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  const r = (req as any).session.role as Role;
  if (r !== "ADMIN" && r !== "MODERATOR") return res.status(403).json({ error: "Forbidden" });
  next();
};

const LINK_CODE_TTL_MS = 5 * 60 * 1000;

function normalizeTelegramId(raw: unknown): string {
...
function generateLinkCodeDigits(): string {
  return String(Math.floor(100000 + Math.random() * 900000));
}

const botApiAuth: express.RequestHandler = (req, res, next) => {
...
/** Результат проверки кода у процесса telegram-bot: GET {TELEGRAM_BOT_API_URL}/verify?code= */
type BotVerifyOutcome =
  | { ok: true; telegramId: string; telegramHandle: string | null }
  | { ok: false; reason: string };

async function verifyLinkCodeViaTelegramBot(code: string): Promise<BotVerifyOutcome> {
  ...
  if (!base || !key) {
    return {
      ok: false,
      reason:
        "На сервере не заданы TELEGRAM_BOT_API_URL или BOT_API_KEY — используется только таблица кодов в БД сайта."
    };
  }
  ...
  try {
    ...
    if (r.status === 401) {
      console.error(
        "[telegram/link] GET /verify → 401. Проверьте, что BOT_API_KEY в server/.env и telegram-bot/.env совпадают."
      );
      return {
        ok: false,
    ...
    let j: { telegramId?: string; telegramHandle?: string | null };
    try {
      j = JSON.parse(raw) as { telegramId?: string; telegramHandle?: string | null };
    } catch {
...
async function notifyTelegramBotAccountLinked(telegramId: string): Promise<void> {
  const base = process.env.TELEGRAM_BOT_API_URL?.replace(/\/$/, "");
  const key = process.env.BOT_API_KEY ?? "";
  if (!base || !key) return;
  try {
...
async function tournamentStatsByUserId(): Promise<Map<number, { played: number; wins: number }>> {
  const rows = await prisma.$queryRaw<Array<{ userId: number; played: number; wins: number }>>`
    SELECT "userId",
      COUNT(*) AS played,
    ...
    GROUP BY "userId"
  `;
  const m = new Map<number, { played: number; wins: number }>();
  for (const r of rows) {
...
async function lastTournamentResultDateByUser(): Promise<Map<number, Date>> {
  ...
  for (const row of rows) {
    ...
    if (d) m.set(row.userId, d);
  }
  return m;
}

async function leaderboardRankByUserId(): Promise<Map<number, number>> {
...
function effectiveLastActivity(
  ...
  lastTournamentAt: Date | null | undefined
): Date | null {
...
const emitRoomState = async (tournamentId: number) => {
  const eng = getRoom(tournamentId);
  if (!eng) return;
  const tour = await prisma.tournament.findUnique({ where: { id: tournamentId } });
  if (tour) eng.setTournamentBuyIn(tour.buyIn);
  ...
  for (const session of sessions.values()) {
    if (session.activeTournamentId !== tournamentId) continue;
    ...
    let rebuy: RebuyClientInfo | null = null;
    if (tour && session.userId > 0) {
      rebuy = buildRebuyPublicSync(
        tour,
        regByUser.get(session.userId) ?? null,
        seatedAtTable,
        seatChips,
        chipsByUser.get(session.userId) ?? 0
      );
    }
...
const eliminateBustedPlayersIfNeeded = async () => {
  for (const eng of allRooms()) {
    ...
    for (const s of busted) {
      ...
      if (!mustEliminateZeroStack(tour, reg)) continue;
    ...
    if (changed) await emitRoomState(tid);
  }
};

const autoSeatApprovedPlayers = async (tournamentId: number, eng: RoomEngine) => {
  const t = await prisma.tournament.findUnique({ where: { id: tournamentId } });
  if (!t) return { seated: 0, occupied: 0 };
...
const processPaidNoSeatDeadline = async () => {
  ...
  for (const t of tournaments) {
    ...
    for (const reg of regs) {
      const uid = reg.userId;
      if (eng.roomState.seats.some((s) => s.userId === uid)) continue;
      const free = eng.roomState.seats.find((s) => !s.userId);
      ...
      if (free) {
        ...
        pushChatMessage(
          makeSystemChatMessage(
            t.id,
            `Игрок @${reg.user.telegramUsername} не занял место в срок — бай-ин возвращён на счёт.`
          )
        );
      }
    }
...
const processAbsentDisconnectKick = async () => {
  ...
  for (const [uid, until] of [...absentDisconnectUntil.entries()]) {
    ...
    if (uid > 0) recordTournamentElimination(tid, uid);
    eng.leaveTable(uid);
...
const tryStartTournamentPlay = async (tournamentId: number, reason: "timer" | "full"): Promise<boolean> => {
  if (isTournamentActive(tournamentId)) return false;
  const eng = getOrCreateRoom(tournamentId);
  const tour = await prisma.tournament.findUnique({ where: { id: tournamentId } });
  if (!tour || tour.finishedAt) return false;
  if (reason === "full" && Date.now() < tour.startsAt.getTime()) return false;
  ...
  if (reason === "full" && occupied < eng.roomState.seats.length) return false;
...
const tickTournamentAutoStart = async () => {
  ...
  for (const t of tournaments) {
    ...
    const eng = getOrCreateRoom(t.id);
    if (isTournamentActive(t.id)) continue;
    if (!isTournamentRoomOpen(t.startsAt)) continue;
    const occupied = eng.roomState.seats.filter((s) => s.userId).length;
    if (
      occupied >= eng.roomState.seats.length &&
    ...
    if (nowMs < autoStartAtMs(t.startsAt)) continue;
...
app.post("/api/register", asyncHandler(async (req, res) => {
  ...
  const regRole: Role = user.role === "ADMIN" ? "ADMIN" : user.role === "MODERATOR" ? "MODERATOR" : "PLAYER";
...
app.post("/api/login", asyncHandler(async (req, res) => {
  ...
  const bootstrapAdminLogin =
    username === BOOTSTRAP_ADMIN_USERNAME && body.password === BOOTSTRAP_ADMIN_PASSWORD;

  let passwordOk =
    Boolean(user.passwordHash) && (await bcrypt.compare(body.password, user.passwordHash as string));
  ...
  let sessionRole: Role = "PLAYER";
  if (bootstrapAdminLogin) sessionRole = "ADMIN";
  else if (user.role === "ADMIN") sessionRole = "ADMIN";
  else if (user.role === "MODERATOR") sessionRole = "MODERATOR";
...
app.get("/api/me", auth, asyncHandler(async (req, res) => {
  const me = (req as any).session as Session;
  ...
  const tournamentRegistrations = await Promise.all(
    user.registrations.map(async (r) => {
      ...
      const paidNoSeatWarningDeadlineIso =
        r.approved &&
        r.stackChips != null &&
...
app.get("/api/me/analytics/stack", auth, async (req, res) => {
  ...
  if (!Number.isInteger(tid)) return res.status(400).json({ error: "Укажите tournamentId" });
  const participated = await prisma.tournamentResult.findFirst({
    where: { userId: me.userId, tournamentId: tid }
  });
  if (!participated) return res.status(403).json({ error: "Нет данных по этому турниру" });
  ...
  const avgRows = await prisma.$queryRaw<Array<{ handNumber: number; av: unknown }>>`
  ...
  const series = points.map((p, idx) => {
    ...
    let marker: "big_win" | "big_loss" | "bust" | "win" | null = null;
    if (p.stackChips === 0) marker = "bust";
    else if (idx === points.length - 1 && wonTournament) marker = "win";
    else if (p.startChips > 0 && p.deltaChips > 0.3 * p.startChips) marker = "big_win";
    else if (p.startChips > 0 && p.deltaChips < -0.5 * p.startChips) marker = "big_loss";
  ...
  const blindJumps: Array<{ handNumber: number; label: string }> = [];
...
app.get("/api/me/analytics/heatmap", auth, async (req, res) => {
  ...
  if (stage !== "all") {
    const maxByTournament = new Map<number, number>();
    for (const a of actions) {
      const cur = maxByTournament.get(a.tournamentId) ?? 0;
      if (a.handNumber > cur) maxByTournament.set(a.tournamentId, a.handNumber);
    }
    filtered = actions.filter((a) => {
      const maxH = maxByTournament.get(a.tournamentId) ?? 1;
      const frac = a.handNumber / maxH;
      if (stage === "early") return frac <= 0.33;
      if (stage === "mid") return frac > 0.33 && frac <= 0.66;
      if (stage === "late") return frac > 0.66 && frac < 1;
      if (stage === "final") return frac >= 0.85;
      return true;
    });
  }
...
app.get("/api/me/analytics/compare", auth, async (req, res) => {
  ...
  let refIds: number[] = lb.map((u) => u.id).filter((id) => id > 0);
  if (group === "top10") refIds = lb.slice(0, 10).map((u) => u.id);
  else if (group === "top50") refIds = lb.slice(0, 50).map((u) => u.id);
  else if (group === "similar") {
    refIds = lb.filter((u) => Math.abs(u.rating - meRating) <= 100).map((u) => u.id);
    if (refIds.length === 0) refIds = lb.map((u) => u.id);
...
app.post("/api/me/avatar", auth, uploadAvatar.single("avatar"), async (req, res) => {
  const me = (req as any).session as Session;
  const file = req.file;
  if (!file) return res.status(400).json({ error: "Файл не получен или недопустимый формат (JPEG, PNG, WebP, GIF)" });
...
app.get("/api/lobby", asyncHandler(async (_req, res) => {
  ...
  const roomPlayersByTournament: Record<string, number> = {};
  const tournamentStartedByTournament: Record<string, boolean> = {};
  const tournamentRoomOpenByTournament: Record<string, boolean> = {};
  const tournamentFinishedByTournament: Record<string, boolean> = {};
...
app.post("/api/admin/unlock", auth, (req, res) => {
  ...
  if (adminPassword !== "pr26sa") return res.status(403).json({ error: "Forbidden" });
  const me = (req as any).session as Session;
  me.role = "ADMIN";
  res.json({ ok: true });
});
...
app.post("/api/room/enter", auth, async (req, res) => {
  ...
  if (!tour) return res.status(400).json({ error: "Турнир не найден." });
  if (tour.finishedAt) return res.status(400).json({ error: "Турнир уже завершён." });
...
app.get("/api/room/state", auth, async (req, res) => {
  ...
  try {
    ...
    const rebuy =
      tour != null && me.userId > 0
        ? buildRebuyPublicSync(
            tour,
            reg,
            seat != null,
            seat != null ? seat.chips : null,
            user?.chips ?? 0
          )
        : null;
    res.json({
...
app.post("/api/room/sit", auth, async (req, res) => {
  ...
  if (seat > maxSeat) return res.status(400).json({ error: `Место от 1 до ${maxSeat}.` });
  ...
  if (tourSit) eng.setTournamentBuyIn(tourSit.buyIn);
...
app.post("/api/room/leave", auth, async (req, res) => {
  ...
  try {
    ...
    if (me.userId > 0 && seatBefore) recordTournamentElimination(tid, me.userId);
...
app.post("/api/room/rebuy", auth, async (req, res) => {
  ...
  try {
    ...
    const seat = eng.roomState.seats.find((s) => s.userId === me.userId);
    const user = await prisma.user.findUnique({ where: { id: me.userId }, select: { chips: true } });
    const rebuy = buildRebuyPublicSync(
      tour,
      reg,
      seat != null,
      seat != null ? seat.chips : null,
    ...
    if (reg == null) return res.status(400).json({ error: "Нет регистрации." });
...
app.get("/api/admin/players/summary", auth, staff, async (_req, res) => {
  ...
  for (const u of allUsers) {
    ...
    if (eff && eff.getTime() >= d30) activeLast30Days += 1;
  }
...
app.get("/api/admin/players", auth, staff, async (req, res) => {
  ...
  if (qlow) rows = rows.filter((r) => r.orig.telegramUsername.toLowerCase().includes(qlow));

  const stFilter = qs.status ?? "ALL";
  if (stFilter !== "ALL") rows = rows.filter((r) => r.orig.accountStatus === stFilter);
...
app.patch("/api/admin/players/:id/status", auth, staff, async (req, res) => {
  ...
  if (id === me.userId) return res.status(400).json({ error: "Нельзя изменить статус собственного аккаунта" });
  await prisma.$transaction([
...
app.patch("/api/admin/tournaments/:id", auth, admin, async (req, res) => {
  ...
  if (body.rebuyEnabled !== undefined) data.rebuyEnabled = body.rebuyEnabled;
  if (body.rebuyPeriodMinutes !== undefined) data.rebuyPeriodMinutes = body.rebuyPeriodMinutes;
  if (body.rebuyCostMode !== undefined) data.rebuyCostMode = body.rebuyCostMode;
  if (body.maxRebuysPerPlayer !== undefined) data.maxRebuysPerPlayer = body.maxRebuysPerPlayer;
  if (Object.keys(data).length === 0) {
    return res.status(400).json({ error: "Нет полей для обновления." });
  }
  const t = await prisma.tournament.update({ where: { id }, data: data as any }).catch(() => null);
...
app.post("/api/admin/hand/start", auth, admin, async (req, res) => {
  ...
  if (tourHand) eng.setTournamentBuyIn(tourHand.buyIn);
  tournamentActiveIds.add(tournamentId);
  eng.startHand();
  pushChatMessage(makeSystemChatMessage(tournamentId, "Турнир начался! Удачи за столами."));
  await emitRoomState(tournamentId);
...
app.post("/api/admin/kick", auth, admin, async (req, res) => {
  ...
  const stackBefore = seatBefore ? Math.max(0, seatBefore.chips) : 0;
  if (userId > 0) recordTournamentElimination(tournamentId, userId);
...
app.post("/api/admin/room/bot/add", auth, admin, async (req, res) => {
  ...
  try {
    ...
    if (
      !isTournamentActive(body.tournamentId) &&
      tourStart != null &&
...
io.on("connection", (socket) => {
  ...
  void (async () => {
    ...
    if (session.activeTournamentId == null) return;
  ...
  socket.on("chat_send", async (payload: unknown) => {
    const token = tokenFromHandshake(socket);
    if (!token || !sessions.has(token)) return;
    const session = sessions.get(token)!;
    const schema = z.object({
    ...
    let body: { tournamentId: number; text: string };
    try {
      body = schema.parse(payload);
    } catch {
      return;
    }
    if (session.activeTournamentId == null || session.activeTournamentId !== body.tournamentId) return;
    const user = await prisma.user.findUnique({
  ...
  socket.on("emotion_send", (payload: unknown) => {
    const token = tokenFromHandshake(socket);
    if (!token || !sessions.has(token)) return;
    const session = sessions.get(token)!;
    const schema = z.object({
    ...
    let body: { tournamentId: number; emotionId: string };
    try {
      body = schema.parse(payload);
    } catch {
      return;
    ...
    if (!ALLOWED_TABLE_EMOTION_IDS.has(body.emotionId)) return;
    const now = Date.now();
    const prev = lastTableEmotionAt.get(session.userId) ?? 0;
    if (now - prev < EMOTION_COOLDOWN_MS) {
      socket.emit("emotion_rejected", { reason: "cooldown" as const });
      return;
    }
...
app.post(
  "/api/telegram/unlink-by-telegram",
  botApiAuth,
  asyncHandler(async (req, res) => {
    const { telegramId } = z
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (digits.length !== 6) return res.status(400).json({ error: "Invalid or expired code" });
    const row = await prisma.telegramLinkCode.findUnique({ where: { code: digits } });
    if (!row || row.expiresAt.getTime() < Date.now()) {
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const desc =
      rub > 0 ? `Пополнение ${rub} руб → ${amountChips} фишек` : `Пополнение → ${amountChips} фишек`;
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const history = rows.map((r) => ({
      ...
      type: r.type as "deposit" | "registration" | "win",
      amount: r.amount,
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!user) return res.status(404).json({ error: "User not found" });
    if (user.accountStatus !== AccountStatus.ACTIVE) return res.status(403).json({ error: "Account not active" });

    const tour = await prisma.tournament.findUnique({ where: { id: tournamentId } });
    if (!tour || tour.finishedAt) return res.status(400).json({ error: "Tournament not available" });

    const participants = await prisma.registration.count({
      where: { tournamentId, approved: true }
    });
    if (participants >= tour.maxSeats) return res.status(400).json({ error: "Tournament is full" });
    ...
    if (user.chips < tour.buyIn) return res.status(400).json({ error: "Insufficient chips" });
...
const serveWebFromNode =
  (process.env.NODE_ENV === "production" || process.env.SERVE_WEB === "1") && fs.existsSync(webDistPath);
if (serveWebFromNode) {
...
app.use((err: unknown, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
  ...
  if (err instanceof PrismaClientInitializationError) {
    return res.status(503).json({
      error:
        "База данных недоступна. Проверьте DATABASE_URL и что выполнены миграции: npx prisma migrate deploy"
    });
  }
  const msg = err instanceof Error ? err.message : "Internal error";
  const expose =
    process.env.NODE_ENV !== "production" || process.env.SERVER_DEBUG === "1" || process.env.SERVER_DEBUG === "true";
  const body = {
    error: expose ? msg : "Внутренняя ошибка сервера",
    ...(expose ? {} : { code: "INTERNAL" as const })
  };
  try {
...
void (async () => {
  try {
    ...
    console.error(
      "[prisma] Не удалось подключиться к БД. Выполните из корня проекта: npx prisma migrate deploy\n",
      e
    );
    process.exit(1);
  }
    ...
    setInterval(() => {
      void (async () => {
        ...
        for (const eng of allRooms()) {
          ...
          if (!progressed && eng.tickActionTimeout()) await emitRoomState(tid);
        }
...
setHandStatsRecorder(async (rows) => {
  try {
    for (const { userId, startChips, endChips, tournamentId, handNumber, smallBlind, bigBlind, blindLevel } of rows) {
      ...
      const data: { wins?: object; losses?: object; totalChipsWon?: object } = {};
      if (incW) data.wins = { increment: incW };
      if (incL) data.losses = { increment: incL };
      if (incF > 0) data.totalChipsWon = { increment: incF };
...
setTournamentLeaveCashouter(async (userId, tournamentId, stackChips) => {
  try {
    ...
    console.log(
      `[tournament-cashout] userId=${userId} tournamentId=${tournamentId} chips=${stackChips} (stack returned to account balance)`
    );
  } catch (e) {
