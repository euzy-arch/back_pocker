...
export function parsePeriod(q: Record<string, unknown>): PeriodFilter {
...
export async function tournamentResultsInPeriod(
  prisma: PrismaClient,
  userId: number,
  period: PeriodFilter
) {
...
export async function leaderboardRows(prisma: PrismaClient) {
...
export async function userTournamentMetrics(
  prisma: PrismaClient,
  userId: number,
  period: PeriodFilter
) {
...
export async function preflopSkillMetrics(
  prisma: PrismaClient,
  userId: number,
  period: PeriodFilter
) {
  ...
  if (actions.length === 0) {
    return {
      pfrPct: null as number | null,
      threeBetPct: null as number | null,
      callFacingPct: null as number | null,
      sampleSize: 0
    };
  }
  ...
  const threeBetPct =
    actions.length > 0 ? Math.round((1000 * threeBetCount) / actions.length) / 10 : null;
...
export function compareTip(
  ...
  if (userVal == null || refAvg == null) return null;
