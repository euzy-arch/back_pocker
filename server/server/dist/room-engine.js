...
let handStatsRecorder = null;
let analyticsPreflopCollector = null;
let tournamentStackPersist = null;
/** Возврат турнирного стека на баланс аккаунта при снятии игрока (leave / kick / конец олл-ин leave). */
let tournamentLeaveCashout = null;
export const setHandStatsRecorder = (fn) => {
    handStatsRecorder = fn;
};
export const setAnalyticsPreflopCollector = (fn) => {
    analyticsPreflopCollector = fn;
};
export const setTournamentStackPersister = (fn) => {
    tournamentStackPersist = fn;
};
export const setTournamentLeaveCashouter = (fn) => {
    tournamentLeaveCashout = fn;
};
let handWinnerChatNotifier = null;
export const setHandWinnerChatNotifier = (fn) => {
...
export const isBotUserId = (id) => id < 0;
function positionBucket(seat, dealerSeat, n) {
    if (n < 2)
        return "EP";
...
export function createRoomEngine(tournamentId) {
    let handStartStacks = new Map();
    let handsDealt = 0;
    /** Последняя завершённая раздача (для экрана победы турнира); не сбрасывается до resetGame. */
    let lastPublicHandOutcome = null;
    let analyticsPreflopBuffer = [];
    ...
    let pendingLeaveAfterHand = new Set();
    let pendingJoinAfterHand = new Map();
    let lastActionByUser = new Map();
    /** Бай-ин турнира (фишки): SB = 0.5%, BB = 1%; null — дефолтные блайнды стола. */
    let tournamentBuyIn = null;
    /** Момент старта «турнирных часов» блайндов (первая раздача после настройки бай-ина). */
    let blindClockStartedAt = null;
    let lastAppliedBlindLevel = 0;
    const baseBlindsFromBuyIn = (buyIn) => {
    ...
    const applyBlindsForLevel = (level) => {
        ...
        if (!roomState.handActive)
            roomState.minRaiseIncrement = bb;
    };
    let roomState = {
        open: false,
        password: "",
    ...
    const emitHandWinnerChatLine = (text) => {
        if (!handWinnerChatNotifier || !text)
            return;
        void Promise.resolve(handWinnerChatNotifier(tournamentId, text)).catch(() => { });
    };
    const buildShowdownWinnerChat = (sr) => {
    ...
    const nextSeatNum = (fromSeat) => (fromSeat % seatCount()) + 1;
    const nextOccupiedFrom = (fromSeat) => {
        const n = seatCount();
        let s = nextSeatNum(fromSeat);
        for (let k = 0; k < n; k += 1) {
            if (roomState.seats[s - 1].userId)
                return s;
            s = nextSeatNum(s);
        }
        return null;
    ...
    const occupiedSeatsOrdered = () => roomState.seats.filter((s) => s.userId && s.chips > 0).sort((a, b) => a.seat - b.seat);
    const nonFoldedWithCards = () => roomState.seats.filter((s) => s.userId && !s.folded);
    const canVoluntarilyAct = (s) => Boolean(s.userId && s.chips > 0 && !s.folded && !s.allIn);
    const nextToActFrom = (fromSeat) => {
        const n = seatCount();
        let s = nextSeatNum(fromSeat);
        for (let k = 0; k < n; k += 1) {
            const st = roomState.seats[s - 1];
            if (canVoluntarilyAct(st))
                return s;
            s = nextSeatNum(s);
        }
        return null;
    ...
    const getFirstToActPostFlop = () => nextToActFrom(roomState.dealerSeat);
    const getRecentAction = (uid) => {
        const rec = lastActionByUser.get(uid);
        if (!rec)
            return null;
        if (Date.now() - rec.at > ACTION_BADGE_MS)
            return null;
        return rec;
    };
    const setDeadline = () => {
        if (roomState.handActive && roomState.turnSeat)
            roomState.turnDeadlineAt = Date.now() + ACTION_MS;
        else
            roomState.turnDeadlineAt = null;
    };
    const tickActionTimeout = () => {
        if (roomState.showdownReveal)
            return false;
        if (!roomState.handActive || !roomState.turnSeat || !roomState.turnDeadlineAt)
            return false;
        if (Date.now() < roomState.turnDeadlineAt)
            return false;
    ...
    const straightHighCard = (ranksDesc) => {
    ...
    const evaluate7 = (cards) => {
        ...
        if (flushRanks.length)
            return [5, ...Array.from(new Set(flushRanks)).slice(0, 5)];
    ...
    const comboTitleRuEn = (score) => {
        ...
        if (cat === 3)
            return "Сет / Three of a Kind";
        if (cat === 2)
            return "Две пары / Two Pair";
        if (cat === 1)
            return "Пара / One Pair";
        return "Старшая карта / High Card";
    };
    /** Короткое русское название комбинации из строки вида «Флэш / Flush». */
    const comboShortRuFromTitle = (fullTitle) => {
        const i = fullTitle.indexOf(" / ");
        if (i === -1)
            return fullTitle.trim();
        return fullTitle.slice(0, i).trim();
    };
    const formatComboDetail = (score) => {
        const cat = score[0] ?? 0;
        if (cat === 9)
            return "Роял-флэш";
        if (cat === 8)
            return `Стрит-флэш, старшая карта — ${rankWordHigh(score[1] ?? 0)}`;
        if (cat === 7) {
            const q = score[1] ?? 0;
            const k = score[2] ?? 0;
        ...
        if (cat === 4) {
            const hi = score[1] ?? 0;
            if (hi === 5)
                return "Стрит от туза до пятёрки (колесо)";
    ...
    const buildWinnerAnnouncementLines = (winnerIds, boardCards) => {
        const tag = (uid) => {
        ...
        if (winnerIds.length === 1)
            winnerLine = `Игрок ${tags[0]} выиграл`;
        else if (winnerIds.length === 2)
            winnerLine = `Игроки ${tags[0]} и ${tags[1]} выиграли (делёж банка)`;
        else
            winnerLine = `Победители делят банк: ${tags.join(", ")}`;
        ...
        if (parts.length === 1) {
            ...
            if (same)
                reasonLine = `${formatComboDetail(first)} — делёж банка`;
            else
                reasonLine = parts.map((p) => `@${p.username}: ${formatComboDetail(p.score)}`).join("; ");
        }
    ...
    const collectStreetToSidePots = () => {
        ...
        for (const level of levels) {
            ...
            if (amount > 0 && eligibleSeats.length > 0)
                roomState.sidePots.push({ amount, eligibleSeats });
            prev = level;
        }
    ...
    const addToBet = (seat, targetToPay) => {
    ...
    const allInShowdownRunout = () => {
        if (roomState.board.length >= 5)
            return false;
        const alive = nonFoldedWithCards();
        if (alive.length <= 1)
            return false;
        if (!alive.every((s) => s.allIn))
            return false;
    ...
    const maybeAutoStartNextHand = () => {
        if (roomState.showdownReveal)
            return;
        if (roomState.handResultOverlay)
            return;
    ...
    const flushPendingSeatJoins = () => {
        if (pendingJoinAfterHand.size === 0)
            return;
        for (const item of [...pendingJoinAfterHand.values()]) {
            ...
            if (!target || target.userId || already)
                continue;
    ...
    const setShowdownChoice = (userId, show) => {
        const sr = roomState.showdownReveal;
        if (!sr || !sr.winnerIds.includes(userId))
            throw new Error("Нет выбора показа для этого игрока");
        if (sr.winnerChoice[userId] !== "pending")
            throw new Error("Решение уже принято");
        sr.winnerChoice[userId] = show ? "show" : "muck";
    };
    const tickShowdownPhase = () => {
        const sr = roomState.showdownReveal;
        if (!sr)
            return false;
        if (Date.now() < sr.deadlineAt)
            return false;
    ...
    const tickHandResultPause = () => {
        if (roomState.showdownReveal)
            return false;
        const o = roomState.handResultOverlay;
        if (!o || Date.now() < o.dismissAt)
            return false;
        roomState.handResultOverlay = null;
        advanceDealerButton();
        maybeAutoStartNextHand();
    ...
    function applyActionInternal(userId, action, amount = 0) {
        if (roomState.showdownReveal)
            throw new Error("Дождитесь окончания показа карт.");
        if (!roomState.handActive)
            throw new Error("Раздача не начата");
        const seat = roomState.seats.find((s) => s.userId === userId);
        if (!seat)
            throw new Error("Игрок не за столом");
        if (roomState.turnSeat !== seat.seat)
        ...
        if (action === "FOLD") {
            ...
            if (amount <= toCall)
                throw new Error("Сумма должна быть больше колла");
            addToBet(seat, Math.min(amount, seat.chips));
            const newBet = seat.bet;
            const increment = newBet - tableBetBefore;
            const shortAllIn = seat.allIn && newBet < tableBetBefore + roomState.minRaiseIncrement;
            if (!shortAllIn && increment < roomState.minRaiseIncrement)
                throw new Error("Минимальный рейз не соблюдён");
            if (newBet <= tableBetBefore)
                throw new Error("Рейз должен поднять ставку");
            roomState.currentBet = newBet;
            roomState.minRaiseIncrement = Math.max(roomState.minRaiseIncrement, increment);
            actedThisStreet.clear();
        }
        ...
        const actionBadge = (() => {
            ...
            if (action === "ALL_IN")
                return { kind: "allin", label: "ALL-IN" };
            return { kind: "raise", label: `RAISE ${seat.bet}` };
        })();
    ...
    const applyAction = (userId, action, amount = 0) => {
    ...
    const tickBotAction = () => {
        if (roomState.showdownReveal)
            return false;
        if (!roomState.handActive || roomState.turnSeat == null)
            return false;
        const seat = roomState.seats.find((s) => s.seat === roomState.turnSeat);
        if (!seat?.userId || !isBotUserId(seat.userId))
            return false;
        const toCall = Math.max(roomState.currentBet - seat.bet, 0);
        const canRaise = seat.chips > toCall;
        const minRaisePay = Math.min(seat.chips, toCall + roomState.minRaiseIncrement);
        const randomRaisePay = Math.min(seat.chips, Math.max(minRaisePay, toCall + randomInt(1, Math.max(roomState.bigBlind, 2) + 1)));
        const r = Math.random();
        try {
            if (toCall === 0) {
                if (!canRaise || r < 0.72)
                    applyActionInternal(seat.userId, "CHECK");
                else if (r < 0.95)
    ...
    const startHand = () => {
        ...
        handsDealt += 1;
        if (tournamentBuyIn != null) {
            if (blindClockStartedAt == null)
                blindClockStartedAt = Date.now();
        ...
        for (const s of roomState.seats) {
            if (s.userId)
                handStartStacks.set(s.userId, s.chips);
        }
    ...
    const recordEarlyLeaveHandStats = (userId) => {
    ...
    const resolveShowdownForLeaver = (sr, userId) => {
    ...
    const flushPendingLeavesAfterHand = () => {
        if (pendingLeaveAfterHand.size === 0)
            return;
        for (const uid of [...pendingLeaveAfterHand]) {
            if (roomState.seats.some((s) => s.userId === uid))
                clearOccupantFromSeat(uid, "forfeit");
        }
        pendingLeaveAfterHand.clear();
    };
    const applyPendingLeavesToShowdown = (sr) => {
        if (pendingLeaveAfterHand.size === 0)
            return;
        for (const uid of [...pendingLeaveAfterHand]) {
    ...
    const clearOccupantFromSeat = (userId, mode) => {
        const seat = roomState.seats.find((s) => s.userId === userId);
        if (!seat)
            return;
        ...
        if (mode === "forfeit" && stack > 0 && !isBotUserId(uid)) {
            void Promise.resolve(tournamentLeaveCashout?.(uid, tournamentId, stack)).catch((e) => console.error("tournamentLeaveCashout", e));
        }
        seat.userId = null;
        seat.username = null;
        seat.hand = [];
        ...
        if (isBotUserId(uid))
            return;
        if (mode === "persist")
            void tournamentStackPersist?.([{ userId: uid, chips: stack }]);
    ...
    const creditChipsToSeat = (userId, amount) => {
        if (!Number.isFinite(amount) || amount <= 0)
            throw new Error("Сумма докупки должна быть положительной");
        const seat = roomState.seats.find((s) => s.userId === userId);
        if (!seat)
            throw new Error("Игрок не за столом");
    ...
    const joinSeat = (seat, userId, username, chips) => {
        if (seat < 1 || seat > roomState.seats.length)
            throw new Error("Неверное место");
        if (roomState.handActive) {
        ...
        if (prev)
            clearOccupantFromSeat(userId, "persist");
    ...
    const leaveSeat = (userId) => {
        clearOccupantFromSeat(userId, "forfeit");
    };
    const setTournamentBuyIn = (buyIn) => {
    ...
    const tickBlindLevel = (now) => {
        if (tournamentBuyIn == null || blindClockStartedAt == null)
            return false;
        if (roomState.handActive || roomState.showdownReveal || roomState.handResultOverlay)
            return false;
        const level = getCurrentBlindLevel(now);
        if (level <= lastAppliedBlindLevel)
            return false;
    ...
    return {
        ...
        isHandInProgress: () => roomState.handActive || roomState.showdownReveal || Boolean(roomState.handResultOverlay)
    };
}
